...
REPO_DIR="/Users/subrotodebnath/Desktop/ClueNote/clue-notes"
...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "ERROR: $REPO_DIR does not appear to be a git repo."
  exit 1
fi

mkdir -p docs

cat > "docs/chat-history.md" <<'MD'
...
# Stage & commit
git add docs/chat-history.md docs/requirements.md
git commit -m "docs: add chat history and requirements for Clue Notes (MVP)"
echo "Files created and committed. Run 'git push' to push to remote if desired."